...
for name in sho_names:
    # 虚実だけは 0.5 (中庸) をデフォルトに、他は 0.0 に設定
    default_val = 0.5 if name == '虚実' else 0.0
    sho_input[name] = st.sidebar.slider(f"{name}", 0.0, 1.0, default_val, key=f"slider_{name}")
...
def create_patient_vec(sho, raw, age):
    p = {k: 1e-9 for k in yakuno_cols} # ゼロ除算回避のための極小値
    ...
    return np.array([p[k]**2 for k in yakuno_cols]) # 二乗でシャープ化

patient_vec = create_patient_vec(sho_input, raw_input, age)
yakuno_data = df_full[yakuno_cols].fillna(0).values

# マッチング計算
raw_sim = cosine_similarity([patient_vec], yakuno_data)[0]
spec_bonus = 1.0 / (np.sum(yakuno_data, axis=1) + 1.0)
df_full['一致度'] = raw_sim * (1.0 + spec_bonus * 0.2)
# 全体の中での相対的な一致度を 0-1 にスケール
df_full['一致度'] = (df_full['一致度'] - df_full['一致度'].min()) / (df_full['一致度'].max() - df_full['一致度'].min() + 1e-9)
