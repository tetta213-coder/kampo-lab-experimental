...
# 感度の初期値を大幅にアップ
sensitivity = st.sidebar.slider("星の移動感度 (幾何学)", 5.0, 100.0, 40.0)
...
if engine == "幾何学的射影 (地図固定)":
    ...
    # 【改良】上位10処方のみを抽出して加重平均（金縛りを解く鍵）
    near_indices = dists.argsort()[:10]
...
tab_cos, tab_dist = st.tabs(["24Dパターンの合致", "2D地図上の近接度"])
with tab_cos:
...
fig = px.scatter(df_calc, x='x', y='y', text='formula', color='cos_sim', color_continuous_scale='Viridis', hover_name='formula', height=700)
